...
public readonly struct BigFloat : IComparable, IComparable<BigFloat>, IEquatable<BigFloat>
{
    ...
    public static BigFloat Parse(string value)
    {
        ...
        var num = value.IndexOf(numberFormat.NumberDecimalSeparator, StringComparison.Ordinal);
    ...
    public string ToString(int precision, bool trailingZeros = false)
    {
        ...
        var numberFormat = Thread.CurrentThread.CurrentCulture.NumberFormat;
        var bigInteger1 = BigInteger.DivRem(bigFloat.Numerator, bigFloat.Denominator, out var remainder);
    ...
    {
        var bigFloat = Factor(this);
        var bigInteger = BigInteger.DivRem(bigFloat.Numerator, bigFloat.Denominator, out var remainder);
        if (remainder == 0L)
